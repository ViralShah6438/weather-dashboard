...
- ✅ Audited and updated all packages to latest secure versions
- ✅ Removed deprecated packages and APIs
- ✅ Strong typing with TypeScript strict mode
- ✅ Nullable reference types enabled in C#
- ✅ Implicit usings for cleaner C# code

### State Management
- Angular services with RxJS Subjects/BehaviorSubjects for reactive updates
- No external state management library needed for this scope
- Services handle API communication and local caching

### Security Considerations
- CORS configured to only accept requests from authorized origins
- Rate limiting prevents abuse (global, per-window, concurrency limits)
- API key stored in configuration, not in source code
- Input validation on both frontend and backend
- ProblemDetails format for standardized error responses
...
### "Unable to load service index for source https://...nuget..."
- The backend uses a local `NuGet.config` to fetch from nuget.org only
- Verify internet connectivity and proxy settings if needed

### CORS errors in browser console
- Ensure both frontend and backend are running
- Check `appsettings.json` CORS configuration matches your frontend URL
- Frontend should run on `http://localhost:4201` by default

### "City not found" error
- Verify the city name spelling
- Try a major city like "London", "New York", "Tokyo"
- Check that OpenWeatherMap API key is valid in `appsettings.json`
...
cd apps/web
npm run build:prod
# Output: dist/weather-dashboard-web/
```
...
# Configure appsettings.Production.json with environment-specific values
```
...
**Architecture Highlights to Mention (20-30 min presentation)**:
1. **Frontend**: NgModule-based architecture with feature modules (scalable, team-friendly)
2. **State Management**: RxJS Subjects instead of Signals (battle-tested, observable patterns)
3. **Backend**: Layered approach with clear separation of concerns (API, Application, Domain, Infrastructure)
4. **Two-Step Weather Lookup**: Geocoding first ensures accurate coordinates before weather API call
5. **Rate Limiting**: Multi-strategy protection prevents abuse and ensures fair resource usage
6. **Caching**: Reduces duplicate API calls for same cities within time window
7. **Security**: CORS, input validation, environment-based configuration
8. **Testing**: Focused unit tests on critical paths (services, controllers, components)
9. **Error Handling**: Standardized ProblemDetails responses for better debugging

**Challenges & Solutions**:
- Converting scaffold from standalone → NgModule required careful module imports/exports
- Vulnerable package versions fixed through audit and update cycle
- GeoAPI requirement necessitated two-step API call flow (handled with async/await)
- Rate limiting complexity managed through configuration-driven approach
